...
# plik z danymi 
with open('./2024.01.31-15.41.20.dbmeter.txt') as f:
  n_cols = len(f.readline().split("\t"))

# moc sygnału w danym zakresie częstliwości dla każdej sekundy pomiaru
freqData = np.loadtxt('./2024.01.31-15.41.20.dbmeter.txt', skiprows=1, usecols=np.arange(3,n_cols+1))
# poziom głoności w całym paśmie dla każdej sekundy pomiaru
loudnessData = np.loadtxt('./2024.01.31-15.41.20.dbmeter.txt', skiprows=1, usecols=2)
# etykiety danych do wizualiacji
labels = np.loadtxt('./2024.01.31-15.41.20.dbmeter.txt', dtype=str, max_rows=1, usecols=np.arange(2,n_cols))
